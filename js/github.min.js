(function(){'use strict';var XMLHttpRequest,_;if(typeof exports!=='undefined'){XMLHttpRequest=require('xmlhttprequest').XMLHttpRequest;_=require('underscore');btoa=require('btoa')}else{_=window._}
if(typeof window!=='undefined'&&typeof window.XMLHttpRequest!=='undefined'){XMLHttpRequest=window.XMLHttpRequest}
var Github=function(options){var API_URL=options.apiUrl||'https://api.github.com';function _request(method,path,data,cb,raw,sync){function getURL(){var url=path.indexOf('//')>=0?path:API_URL+path;return url+((/\?/).test(url)?'&':'?')+(new Date()).getTime()}
var xhr=new XMLHttpRequest();xhr.open(method,getURL(),!sync);if(!sync){xhr.onreadystatechange=function(){if(this.readyState===4){if(this.status>=200&&this.status<300||this.status===304){cb(null,raw?this.responseText:this.responseText?JSON.parse(this.responseText):!0,this)}else{cb({path:path,request:this,error:this.status})}}}}
...
results.push.apply(results,res);var links=(xhr.getResponseHeader('link')||'').split(/\s*,\s*/g),next=_.find(links,function(link){return/rel="next"/.test(link)});if(next){next=(/<(.*)>/.exec(next)||[])[1]}
